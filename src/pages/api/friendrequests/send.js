import { getServerSession } from "next-auth";
import { authOptions } from "../auth/[...nextauth]";
import prisma from "../../../../server/db/prisma";

const send = async (req, res) => {

    try {

        const session = await getServerSession(req, res, authOptions);

        if (session) {
            const prismaUser = await prisma.user.findUnique({
                where: { email: session.user.email }
            })

            if (!prismaUser) {
                res.status(401).json({ error: 'Unauthorized' });
                return;
            }

            const { method } = req;

            switch (method) {
                case 'POST':
                    const { userId } = req.body;

                    if (!prismaUser.id === userId) {
                        res.status(403).end('You can´t send a friend request to yourself. Loser.');
                        return;
                    };

                    const checkUser = await prisma.user.findUnique({
                        where: {
                            id: userId
                        },
                        include: {
                            friendRequests: true,
                            friendRequestsSent: true,
                            friends: true,
                        }
                    })

                    if (!checkUser) {
                        receiverUser.status(404).end('User does not exist.');
                        return;
                    }

                    let requestExists = false;

                    checkUser.friendRequests.forEach(user => {
                        if (user.id === prismaUser.id) requestExists = true;
                    });
                    checkUser.friendRequestsSent.forEach(user => {
                        if (user.id === prismaUser.id) requestExists = true;
                    });
                    checkUser.friends.forEach(user => {
                        if (user.id === prismaUser.id) requestExists = true;
                    });

                    if (requestExists) {

                        res.status(403).end('Request already exist.');
                        return;

                    }

                    const receiverUser = await prisma.user.update({
                        where: {
                            id: userId,
                        },
                        data: {
                            friendRequests: {
                                connect: {
                                    id: prismaUser.id,
                                },
                            },
                        },
                        include: {
                            friends: true,
                            friendRequests: true,
                            friendRequestsSent: true,
                        }
                    });
                    const senderUser = await prisma.user.update({
                        where: {
                            id: prismaUser.id,
                        },
                        data: {
                            friendRequestsSent: {
                                connect: {
                                    id: userId,
                                },
                            },
                        },
                        include: {
                            friends: true,
                            friendRequests: true,
                            friendRequestsSent: true,
                        }
                    });

                    const notification = await prisma.notification.create({
                        data: {
                            to_id: receiverUser.id,
                            from_id: senderUser.id,
                            type: 'friendrequest',
                        }
                    })

                    res.status(200).json({ receiverUser, senderUser });
                    break;

                default:
                    res.status(405).send();
            }

        } else {

            res.status(403).end('You must be signed in to do this.')

        }

    } catch (e) {

        res.status(500).send({ error: e.message });
        console.log(e.message);

    }
}

export default send;